...
const DataSlice = createSlice({
  ...
  reducers: {
    ...
    updateDataList: (state, action: PayloadAction<DataItem>) => {
      const updatedData = state.Datalist.map(item => (item.data_id === action.payload.data_id ? action.payload : item))
...
export const { getDataList, insertDataList, updateDataList, deleteDataList } = DataSlice.actions
